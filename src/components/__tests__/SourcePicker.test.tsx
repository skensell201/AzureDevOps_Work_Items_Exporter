...
const EMPTY: Selection = { tab: 'backlog', project: '', team: '', level: '', query: '' };
...
describe('SourcePicker (controlled)', () => {
  it('changing project emits cleared dependent selection and fires onProjectSelected', () => {
    ...
    expect(props.onChange).toHaveBeenCalledWith({ tab: 'backlog', project: 'p1', team: '', level: '', query: '' });
    expect(props.onProjectSelected).toHaveBeenCalledWith('p1');
  });

  it('changing team clears level and fires onTeamSelected', () => {
    const { props, getByLabelText } = setup({ tab: 'backlog', project: 'p1', team: '', level: 'L', query: '' });
    fireEvent.change(getByLabelText('Team'), { target: { value: 't1' } });
    expect(props.onChange).toHaveBeenCalledWith({ tab: 'backlog', project: 'p1', team: 't1', level: '', query: '' });
    expect(props.onTeamSelected).toHaveBeenCalledWith('p1', 't1');
  });

  it('loads a backlog using the current value', () => {
    const { props, getByText } = setup({ tab: 'backlog', project: 'p1', team: 't1', level: 'Microsoft.RequirementCategory', query: '' });
  ...
  it('shows the Query tab and loads the selected query', () => {
    const { props, getByText } = setup({ tab: 'query', project: 'p1', team: '', level: '', query: 'q1' });
  ...
  it('has a refresh button on the Query tab that fires onRefreshQueries', () => {
    const { props, getByText } = setup({ tab: 'query', project: 'p1', team: '', level: '', query: '' });
